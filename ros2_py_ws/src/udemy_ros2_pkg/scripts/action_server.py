...
def main(args=None):
    rclpy.init()
    action_server_node=NavigateActionServer()					#function Calling
    print("Action Node Running")

    try:
        while rclpy.ok():
            rclpy.spin_once(action_server_node)					#keyboard interrupt for stopping Node 
